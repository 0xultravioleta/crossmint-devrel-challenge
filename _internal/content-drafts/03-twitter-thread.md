...
> **Owner:** thread-writer
> **Target:** 10 tweets after PM fix (Tweet 3 split + Tweet 9 softened); each under 280 chars, each standalone-readable
> **Depends on:** `00-shared-brief.md`, `01-narrative-lock.md`

Character counts shown in `[N/280]` after each tweet.

**PM fix log (applied 2026-04-10):**
- Tweet 3 split into 3a + 3b so all 4 tool names can appear verbatim per shared brief rule "EXACT names, do not paraphrase"
- Tweet 10 (formerly Tweet 9): dropped `lobster.cash` from the public CTA tag — Fede hasn't reviewed the skill yet, so don't publicly ping his team before the cert path is confirmed privately
